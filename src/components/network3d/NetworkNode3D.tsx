...
import { deviceVisualKindFor } from "./types";
import { DEVICE_BOUNDS, KIND_COLOR, THEME } from "./theme";
import { DeviceVisual3D } from "./devices/DeviceVisual3D";
...
export function NetworkNode3D({ node, onSelect, phase = 0 }: NetworkNode3DProps) {
  ...
  const visualKind = node.visualKind ?? deviceVisualKindFor(node.kind);
  const [boundW, boundH, boundD] = DEVICE_BOUNDS[visualKind];
  ...
  return (
    ...
    >
      ...
      <Text position={[0, boundH / 2 + 0.38, 0]} fontSize={0.26} color={THEME.text} anchorX="center" anchorY="bottom">
        {node.label}
      </Text>
      {node.subLabel && (
        <Text position={[0, -boundH / 2 - 0.21, 0]} fontSize={0.16} color={THEME.border} anchorX="center" anchorY="top">
          {node.subLabel}
        </Text>
      )}
      {node.badges && node.badges.length > 0 && (
        <Text position={[0, -boundH / 2 - 0.21 - (node.subLabel ? 0.24 : 0), 0]} fontSize={0.14} color={THEME.cyanSoft} anchorX="center" anchorY="top">
          {node.badges.join(" · ")}
        </Text>
      )}
    </group>
