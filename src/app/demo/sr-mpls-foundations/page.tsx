...
import { SRF_BRIEFING_NOTES, SRF_BRIEFING_PHASES } from "./briefing";
import { SRF_LESSON_SECTIONS, SrFoundationsLessonGuideContent } from "./LessonGuideContent";
import { SRF_DEEP_DIVE_SECTIONS, SrFoundationsDeepDiveContent } from "./DeepDiveContent";

const GUIDE_TABS: LessonGuideTab[] = [
  { id: "lesson", label: "This Lesson", hint: "R1 → R6 · SRGB 16000 · Node SIDs · Adj-SID 24035", sections: SRF_LESSON_SECTIONS, content: <SrFoundationsLessonGuideContent /> },
  { id: "deep", label: "SR-MPLS Deep Dive", hint: "Segment Routing over MPLS in general", sections: SRF_DEEP_DIVE_SECTIONS, content: <SrFoundationsDeepDiveContent /> },
];
...
export default function SrMplsFoundationsDemo() {
  ...
  const sidMeaning = (label: string) => {
    ...
    const prefix = state.prefixSids.find((p) => p.label === value);
    if (prefix) return `${prefix.router} Node SID`;
    const adj = state.adjSids.find((a) => a.label === value);
    return adj ? `${adj.owner}→${adj.neighbor} Adj-SID, local to ${adj.owner}` : undefined;
  };
  const logicalList = state.segmentList && state.segmentList.length > 1 ? logicalListText(state.segmentList) : undefined;
  ...
  const briefing = currentStep ? resolveBriefing(currentStep.id, currentStep.label, SRF_BRIEFING_PHASES, SRF_BRIEFING_NOTES) : undefined;
    ...
    <div className={focus ? "h-full [&>div]:!h-full [&>div]:rounded-none [&>div]:border-0" : undefined}>
      <GraphTopologyViewer nodes={displayNodes} edges={edges} activeNodeIds={activeNodeIds} bestPathEdgeIds={bestPathEdgeIds} onNodeClick={focus ? selectNode2D : undefined} onEdgeClick={(id) => setSelectedLinkId(id)}>
        {activePacket && packetFrom && packetTo && (() => {
          ...
          return <GraphPacketBubble packet={bubblePacket(activePacket, c)} from={packetFrom} to={packetTo} title={c.title} scope={`${activePacket.from} → ${activePacket.to}`} onSelect={() => setPacketSelected(true)} />;
        })()}
      </GraphTopologyViewer>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-10">
      ...
      <div>
        ...
        <LessonGuideDialog open={guideOpen} onClose={() => setGuideOpen(false)} title="SR-MPLS Foundations" subtitle="SRGB 16000–23999 · Node SIDs 16001–16006 · Adj-SID 24035 (R3→R5)" tabs={GUIDE_TABS} />
      </div>
