...
import { SRV6L_BRIEFING_NOTES, SRV6L_BRIEFING_PHASES } from "./briefing";
import { SRV6L_LESSON_SECTIONS, Srv6L3vpnLessonGuideContent } from "./LessonGuideContent";
import { SRV6L_DEEP_DIVE_SECTIONS, Srv6L3vpnDeepDiveContent } from "./DeepDiveContent";
...
const GUIDE_TABS: LessonGuideTab[] = [
  { id: "lesson", label: "This Lesson", hint: "VRF CUST-A · RD/RT · per-VRF End.DT4/End.DT6 Service SIDs", sections: SRV6L_LESSON_SECTIONS, content: <Srv6L3vpnLessonGuideContent /> },
  { id: "deep", label: "SRv6 L3VPN Deep Dive", hint: "BGP services over SRv6 (RFC 9252) in general", sections: SRV6L_DEEP_DIVE_SECTIONS, content: <Srv6L3vpnDeepDiveContent /> },
];
...
export default function Srv6L3vpnDemo() {
  ...
  // Service SIDs come from each PE's Local SID Table; P1's End.X exists only for the read-only policy preview.
  const sidMeaning = (sid: string) => {
    for (const pe of PE_ROUTERS) {
      const s = localSidTableFor(pe).find((x) => x.sidText === sid);
      if (s) return `${s.owner} ${BEHAVIOR_LABEL[s.behavior]} (${s.vrf})`;
    }
    return sid === P1_ENDX_SID_TEXT ? "P1 End.X" : undefined;
  };
  ...
  const briefing = currentStep ? resolveBriefing(currentStep.id, currentStep.label, SRV6L_BRIEFING_PHASES, SRV6L_BRIEFING_NOTES) : undefined;
  ...
  const graph2D = (focus?: boolean) => (
    <div className={focus ? "relative h-full overflow-hidden [&>div]:!h-full [&>div]:rounded-none [&>div]:border-0" : "relative"}>
      <GraphTopologyViewer nodes={nodes} edges={edges} onNodeClick={focus ? selectNode2D : undefined} activeNodeIds={rrRelayPacket ? [rrRelayPacket.from, rrRelayPacket.to] : activeNodeIds} regions={topoView === "physical" ? GRAPH_REGIONS : []}>
        {(() => {
          const pkt = rrRelayPacket ?? activePacket;
          if (!pkt) return null;
          const from = nodes.find((n) => n.id === pkt.from);
          const to = nodes.find((n) => n.id === pkt.to);
          if (!from || !to) return null;
          const c = calloutFor(pkt);
          return <GraphPacketBubble packet={bubblePacket(pkt, c)} from={from} to={to} title={c.title} scope={`${pkt.from} → ${pkt.to}`} onSelect={rrRelayPacket ? undefined : () => setPacketSelected(true)} />;
        })()}
      </GraphTopologyViewer>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-10">
      ...
      <div>
        ...
        <LessonGuideDialog open={guideOpen} onClose={() => setGuideOpen(false)} title="SRv6 L3VPN" subtitle="PE1 ↔ PE2 over a plain IPv6 core · Service SIDs 2001:db8:100:{1,2}:13:: / :12::" tabs={GUIDE_TABS} />
      </div>
