...
export default function BgpRouteReflectorDemo() {
  ...
  const activeDeviceId = PRIMARY_TRANSITION_ROUTER[currentStep?.id ?? ""] ?? DEVICE_ROUTERS.find((r) => visibleRouterIds.includes(r) && traceFor(r, state, currentStep?.id ?? "", activePacket).activeStageId !== undefined);
  ...
  const focusPosition3D: [number, number, number] | undefined =
    cameraMode === "freeOrbit" ? undefined : cameraMode === "reflection" ? [0, 0, 0] : inDeviceMode ? [0, 0, deviceXray ? -0.2 : 0] : selectedNode3D?.position;
  const eyeOffset3D: [number, number, number] | undefined = cameraMode === "reflection" ? [0.01, 9, 0.01] : inDeviceMode ? (deviceXray ? [0.6, 2.6, 5.2] : [2.1, 1.5, 3.8]) : undefined;
