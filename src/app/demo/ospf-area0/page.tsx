...
export default function OspfArea0Demo() {
  ...
  // re-describing `ospfSteps`/`index` (already-decided data).
  const journeyHopEntries = ospfSteps.slice(0, index + 1).filter((s) => !!s.packet).map((s) => ({ id: s.id, label: s.label }));
          ...
          timeline={
            <div className="space-y-2">
              <HopTimeline
                ...
                onSelectHop={(i) => {
                  const step = ospfSteps.filter((s) => !!s.packet)[i];
                  const p = step?.packet?.(state);
                  const router = p?.to ?? p?.from;
                  if (!router) return;
                  setSelectedNodeId(router as RouterId);
