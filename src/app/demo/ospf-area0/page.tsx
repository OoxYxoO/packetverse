...
export default function OspfArea0Demo() {
  ...
  // Migration" §25) — see bgp-enterprise for the same pattern.
  ...
  // --- Hop Inspector target — explicit selection wins outright, same
  // rule as SR-MPLS/BGP Enterprise: selectedNodeId > effectiveDeviceId >
  // activeDeviceId, decoupled from `inDeviceMode`.
  ...
  // no-packet transition steps — a DOWN→INIT move has no packet of its
  // own), re-describing `ospfSteps`/`index` rather than a separate
  // journey log.
  const journeyStepIndices = ospfSteps
    .map((s, i) => ({ s, i }))
  ...
  const historicalStep = historicalCursor !== undefined ? ospfSteps[historicalCursor] : undefined;
  ...
  /** Detail shown in <ObjectFocusPanel> for a focused stage/packetLayer/interface — see bgp-enterprise for the same pattern. */
  ...
  function handleCameraModeChange(v: CameraMode | "spf") {
    setCameraMode(v);
                ...
                deviceView={
                  inDeviceMode
                    ? {
                        ...
                        onSelectPacket: () => setPacketSelected(true),
                        packetSelected,
                        pipelineTitle: "Conceptual OSPF Control-Plane Pipeline",
                        onFocusObject: setFocusedObject,
                        focusedObjectId: activeFocusedObject?.id,
                      }
                    : undefined
