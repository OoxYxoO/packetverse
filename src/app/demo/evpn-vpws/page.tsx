...
export default function EvpnVpwsDemo() {
  ...
  const activeDeviceId = FABRIC_DEVICES.find((d) => d !== "CORE" && state.journey[state.journey.length - 1]?.device === d);
          ...
          {viewMode === "3d" ? (
            <>
              <NetworkScene3D
                ...
                onSelectNode={(id) => { setSelectedNodeId(id as EvpnVpwsDeviceId); setSelectedRegionId(undefined); setSelectedLinkId(undefined); setPacketSelected(false); }}
                ...
                deviceView={
                  inDeviceMode
                    ? {
                        ...
                        onSelectPacket: () => { setPacketSelected(true); setAutoPlay(false); },
                        packetSelected,
                        pipelineTitle: effectiveDeviceId === "PE3" ? "Conceptual EVPN-VPWS Egress Pipeline" : effectiveDeviceId === "CORE" ? "Conceptual Transport Forwarding Pipeline" : "Conceptual VPWS Control / Forwarding Pipeline",
                      }
                    : undefined
                }
              />
