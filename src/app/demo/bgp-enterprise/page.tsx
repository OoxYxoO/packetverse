...
import { packetCallout } from "@/components/lesson/packetCallout";
import { GraphPacketBubble } from "@/components/network/GraphPacketBubble";
import { BGP_BRIEFING_NOTES, BGP_BRIEFING_PHASES } from "./briefing";
...
export default function BgpEnterpriseDemo() {
  ...
  const historicalStep = historicalCursor !== undefined ? bgpSteps[historicalCursor] : undefined;
  ...
  const briefing = currentStep ? resolveBriefing(currentStep.id, currentStep.label, BGP_BRIEFING_PHASES, BGP_BRIEFING_NOTES) : undefined;
  ...
  const graph2D = (focus?: boolean) => (
    <div className={focus ? "h-full [&>div]:!h-full [&>div]:rounded-none [&>div]:border-0" : undefined}>
      <GraphTopologyViewer onNodeClick={focus ? selectNode2D : undefined} nodes={GRAPH_NODES} edges={edges} activeNodeIds={activeNodeIds} regions={GRAPH_REGIONS} highlightedRegionIds={highlightedRegionIds}>
