...
/** Which fabric device is the primary inspection subject of a no-packet control-plane step — mirrors evpn-irb's PRIMARY_TRANSITION_ROUTER. */
const PRIMARY_TRANSITION_ROUTER: Record<string, "LEAF1" | "SPINE1" | "LEAF2" | "LEAF3"> = {
  "leaf1-local-learn": "LEAF1",
  "enter-leaf2-local-learn": "LEAF2",
  "enter-leaf3-mobility-update": "LEAF3",
  "fault-injected": "LEAF3",
  "repair-challenge": "LEAF3",
  "second-move-optional": "LEAF1",
};

function deviceForStep(stepId: string, packet: PacketVisual | undefined): EvpnMobilityDeviceId | undefined {
  if (packet) return (packet.from ?? packet.to) as EvpnMobilityDeviceId;
...
export default function EvpnMacMobilityDemo() {
  ...
  const explainTargetId = effectiveDeviceId ?? selectedNodeId ?? (followNode3D?.id as EvpnMobilityDeviceId | undefined);
  ...
  const journeyStepIndices = evpnMobilitySteps.map((s, i) => ({ s, i })).filter(({ s, i }) => i <= index && (!!s.packet || PRIMARY_TRANSITION_ROUTER[s.id] !== undefined));
  ...
  const historicalStep = historicalCursor !== undefined ? evpnMobilitySteps[historicalCursor] : undefined;
          ...
          inspector={
            currentStep?.question ? (
              ...
              <GlassPanel strong className="p-5">
                <p className="mb-3 text-sm text-pv-text-muted">A genuine simulation-state change: HOST-A disconnects from LEAF1 and attaches to LEAF2. Its MAC and IP stay exactly the same.</p>
                <Button onClick={() => engine.act({})} disabled={state.moveCount >= 1}>
                  {state.moveCount >= 1 ? "✓ HOST-A Moved To LEAF2" : "[ MOVE HOST-A TO LEAF2 ]"}
                </Button>
              </GlassPanel>
            ) : currentStep?.id === "stale-forwarding-demo" ? (
              <GlassPanel strong className="p-5">
                <p className="mb-3 text-sm text-pv-text-muted">A quick thought experiment: what if LEAF3 had kept forwarding to its old entry?</p>
                <Button variant="secondary" disabled={staleDemoActive} onClick={() => setStaleDemoActive(true)}>
                  {staleDemoActive ? "Sending toward stale LEAF1 entry…" : "Show What Stale Forwarding Would Look Like →"}
                </Button>
              </GlassPanel>
            ) : selectedLinkDetail ? (
              ...
              <div className="space-y-3">
                ...
                <HopInspectorPanel
                  ...
                  onFocusNextHop={(id) => {
                    ...
                    setInspectorSurface("hop");
                    if (cameraMode === "device") setEnteredDeviceId(id as EvpnMobilityDeviceId);
                  }}
                />
