...
export default function SrFlexAlgoDemo() {
  ...
  // --- Generic 3D object-focus sub-state (mirrors mpls-ldp §25 /
  // sr-ti-lfa) — layered ON TOP of `cameraMode`, never a 5th camera
  ...
  // --- Hop Inspector target (mirrors mpls-ldp §23 / sr-ti-lfa) —
  // explicit selection wins outright: selectedNodeId > effectiveDeviceId > activeDeviceId.
  ...
  // `packet` field of its own, so no narrative-hop-steps fallback map
  // is needed (mirrors sr-ti-lfa's identical finding).
  const journeyStepIndices = srFlexAlgoSteps.map((s, i) => ({ s, i })).filter(({ s, i }) => i <= index && !!s.packet);
  ...
  // --- Historical inspection (mirrors mpls-ldp §3/§4 / sr-ti-lfa) —
  // reuses ScenarioEngine's OWN `getStateAt` snapshot. Presentation-only
  // — never calls `engine.goTo()`.
  const historicalState = historicalCursor !== undefined ? engine.getStateAt(historicalCursor) : undefined;
  const historicalStep = historicalCursor !== undefined ? srFlexAlgoSteps[historicalCursor] : undefined;
  ...
  // playback takes priority (mirrors mpls-ldp / sr-ti-lfa).
          ...
          {viewMode3D ? (
            <>
              ...
              {focusMode ? (
                ...
                // modal (one-canvas invariant, see mpls-ldp/sr-ti-lfa).
        ...
        <TopologyFocusMode
          ...
          inspector={
            currentStep?.question ? (
              ...
              // (see sr-ti-lfa's identical finding).
              <>
            ...
            <div className="space-y-2">
              <HopTimeline
                ...
                onSelectHop={(i) => {
                  ...
                  const histStep = srFlexAlgoSteps[entry.index];
