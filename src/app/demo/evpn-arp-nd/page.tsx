...
import { bindingRowsFor, buildArpNdCliCommands, buildSpineCliCommands, evpnRibRowsFor, interfacesFor, linkDetailFor, packetFramesFor, traceFor } from "./deviceTrace";

const FABRIC_DEVICES: ("LEAF1" | "SPINE1" | "LEAF2" | "LEAF3")[] = ["LEAF1", "SPINE1", "LEAF2", "LEAF3"];

/** Which fabric device is the primary inspection subject of a no-packet control-plane step — mirrors evpn-bum's PRIMARY_TRANSITION_ROUTER. */
const PRIMARY_TRANSITION_ROUTER: Record<string, "LEAF1" | "SPINE1" | "LEAF2" | "LEAF3"> = {
  "leaf1-classify-bum": "LEAF1",
  "flood-replicate": "LEAF1",
  "flood-delivered": "SPINE1",
  "enter-leaf1-suppression-pipeline": "LEAF1",
  "fault-injected": "LEAF1",
  "repair-challenge": "LEAF1",
};

/** Sender-priority device resolution — every trace branch here is keyed off the ACTING device (mirrors evpn-bum/evpn-mac-mobility — see ARCHITECTURE.md §18). */
function deviceForStep(stepId: string, packet: PacketVisual | undefined): EvpnArpNdDeviceId | undefined {
  if (packet) return (packet.from ?? packet.to) as EvpnArpNdDeviceId;
...
const REPAIR_OPTIONS = [
  ...
  { id: "disable-flooding", label: "Disable BUM flooding for this VNI" },
  { id: "flap-uplink", label: "Flap LEAF3's uplink to SPINE1" },
  { id: "repair-binding", label: "Restore the missing IP information in HOST-B's MAC/IP binding" },
];
const WRONG_FEEDBACK: Record<string, string> = {
  "restart-evpn": "The session is already Established and the MAC route already arrived — this isn't a session problem.",
  "disable-flooding": "Flooding is legitimate fallback behavior, not the bug — disabling it would break connectivity for every case suppression can't answer, which is worse, not better.",
  "flap-uplink": "Underlay and VTEP reachability are already healthy — HOST-B's MAC route already arrived correctly.",
};

export default function EvpnArpNdDemo() {
  const { engine, snapshot } = useScenarioEngine<EvpnArpNdState>(createEvpnArpNdState(), evpnArpNdSteps);
  ...
  const [selectedNodeId, setSelectedNodeId] = useState<EvpnArpNdDeviceId | undefined>(undefined);
  const [cameraMode, setCameraMode] = useState<CameraMode>("overview");
  const [enteredDeviceId, setEnteredDeviceId] = useState<EvpnArpNdDeviceId | undefined>(undefined);
  ...
  const showPlanes = index >= evpnArpNdSteps.findIndex((s) => s.id === "arp-broadcast");
  const showTroubleshootLayers = index >= evpnArpNdSteps.findIndex((s) => s.id === "break-intro");
  const showBeforeAfter = index >= evpnArpNdSteps.findIndex((s) => s.id === "before-after-comparison") && index < evpnArpNdSteps.findIndex((s) => s.id === "break-intro");
  ...
  const nodes3D = nodes3DBase.map((n) => {
    ...
    else if (visitedDevices.has(n.id as EvpnArpNdDeviceId)) status = "onPath";
    const badges = n.id === "HOST-B" ? [`ON ${state.hostBLocation}`] : n.id === "LEAF1" && state.suppressionEnabled ? ["SUPPRESSION"] : undefined;
  ...
  const links3D: Link3DData[] = physicalAugmented.edges.map((e) => ({
    ...
    onPath: visitedDevices.has(e.a as EvpnArpNdDeviceId) && visitedDevices.has(e.b as EvpnArpNdDeviceId),
  }));
  ...
  // Replication camera model (mirrors evpn-bum): during replication, the camera/scene shows either the
  // replication overview (all copies animating in parallel via FloodCopy3D — the default) or, once the
  // learner clicks one specific copy, that copy's own packet inspector — it never fakes single-packet Follow.
  ...
  const isFabricDevice = (id: EvpnArpNdDeviceId | undefined): id is "LEAF1" | "SPINE1" | "LEAF2" | "LEAF3" => id === "LEAF1" || id === "SPINE1" || id === "LEAF2" || id === "LEAF3";
  ...
  const journeyStepIndices = evpnArpNdSteps.map((s, i) => ({ s, i })).filter(({ s, i }) => i <= index && (!!s.packet || PRIMARY_TRANSITION_ROUTER[s.id] !== undefined));
  ...
  const historicalStep = historicalCursor !== undefined ? evpnArpNdSteps[historicalCursor] : undefined;
  ...
  const diagnosticLayers: DiagnosticLayer[] = [
    ...
    { label: "Type-2 MAC Route", status: "healthy" },
    { label: "Remote MAC", status: "healthy" },
    { label: "Type-2 IP Information", status: state.faultActive ? "failing" : "healthy" },
    { label: "MAC/IP Binding", status: state.faultActive ? "failing" : "healthy" },
    { label: "ARP Suppression", status: state.faultActive ? "failing" : state.suppressionEnabled ? "healthy" : "unknown" },
    { label: "Fallback BUM Discovery", status: state.faultActive ? "healthy" : "unknown" },
    { label: "Final Unicast Connectivity", status: "healthy" },
  ];

  useEffect(() => {
    if (isComplete) {
      completeLesson("evpn-arp-nd-suppression", 350);
      unlockAchievement("broadcast-tamer");
    }
  ...
  const explorerTabsFor = (device: "LEAF1" | "SPINE1" | "LEAF2" | "LEAF3"): DeviceExplorerTab[] => {
    ...
    if (device === "SPINE1") {
      return [
        ...
        { id: "packet", label: "Packet", content: devicePacketForTab ? <PacketInspector packet={devicePacketForTab} /> : <p className="text-xs text-pv-text-faint">No single packet at this device right now — click a VXLAN copy in the 3D view during replication to inspect it.</p> },
    ...
    return [
      ...
      { id: "mac-evpn", label: "MAC / EVPN", content: <EvpnRibViewer title={device} rows={evpnRibRowsFor(state, device)} /> },
      { id: "bindings", label: "MAC-IP Bindings", content: <KeyValueTab rows={bindingRowsFor(state, device)} /> },
      { id: "arp-nd", label: "ARP / ND", content: <KeyValueTab rows={device === "LEAF1" ? [{ label: "Suppression", value: state.suppressionEnabled ? "Enabled" : "Not yet introduced" }, { label: `Last lookup (${HOST_B_IP})`, value: state.faultActive ? "Miss — fallback to BUM" : state.suppressionEnabled ? "Hit — local proxy reply" : "n/a" }] : [{ label: "Suppression", value: "Only LEAF1 is the ingress VTEP in this lesson's flow" }]} /> },
      { id: "packet", label: "Packet", content: devicePacketForTab ? <PacketInspector packet={devicePacketForTab} /> : <p className="text-xs text-pv-text-faint">No single packet at this device right now — click a VXLAN copy in the 3D view during replication to inspect it.</p> },
      {
        id: "control",
        label: "Control Plane",
      ...
      { id: "cli", label: "CLI", content: <CLIOutputPanel commands={buildArpNdCliCommands(state, device)} /> },
    ];
  };
  ...
  const floodCopyInspector = selectedReplicaPacket && (
    <GlassPanel strong className="space-y-3 p-5">
      ...
      <p className="text-xs text-pv-text-muted">One of {state.replicas.length} independent VXLAN copies LEAF1 created from the single original ARP broadcast — HOST-A never transmitted more than one original request.</p>
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-10">
      <div className="mb-6">
        <Badge tone="cyan" className="mb-3">EVPN ARP/ND Suppression · Local Proxy Reply</Badge>
        <h1 className="text-2xl font-semibold text-pv-text sm:text-3xl">Why Ask The Fabric What You Already Know?</h1>
        <p className="mt-2 max-w-3xl text-sm text-pv-text-muted">
          LEAF1 already knows HOST-B&apos;s MAC/IP binding from ordinary EVPN Type 2. Watch it answer HOST-A&apos;s ARP
          request locally instead of flooding the request across every remote VTEP.
        </p>
      </div>

      <div className="mb-4 grid gap-2 sm:grid-cols-4">
        {[
          { q: "WHAT is ARP suppression?", a: "The ingress VTEP answers ARP/ND locally from its own EVPN MAC/IP binding, instead of flooding the request as BUM." },
          { q: "WHAT enables it?", a: "The same EVPN Type 2 MAC/IP route already taught in an earlier lesson — nothing new is advertised for this." },
          { q: "DOES BUM disappear?", a: "No — flooding remains the correct fallback whenever a valid binding doesn't exist." },
          { q: "KEY LESSON?", a: "An optimization can fail while connectivity itself keeps working through legitimate fallback flooding." },
        ].map((item) => (
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        <TopologyModeSwitcher options={[{ value: "physical", label: "Physical Topology" }, { value: "logical", label: "Logical (Control→Discovery→Data)" }, { value: "3d", label: "3D View" }]} value={viewMode} onChange={setViewMode} />
        {viewMode === "3d" && (
          <>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_400px]">
        <div className="space-y-6">
          {viewMode === "3d" ? (
            <>
              ...
              <NetworkScene3D
                ...
                onSelectLink={(id) => { setSelectedLinkId(id); setSelectedNodeId(undefined); setSelectedFloodCopyId(undefined); setPacketSelected(false); }}
                ...
                deviceView={
                  inDeviceMode
                    ? {
                        ...
                        onSelectPacket: () => { setPacketSelected(true); setAutoPlay(false); },
                        packetSelected,
                        pipelineTitle: effectiveDeviceId === "LEAF1" ? (state.suppressionEnabled ? "Conceptual ARP Suppression Pipeline" : "Conceptual BUM Forwarding Pipeline") : "Conceptual Underlay Forwarding Pipeline",
              ...
              {packetSelected && activePacket && (
                <PacketDetailPanel
                  ...
                  direction="HOST-A → LEAF1 → (suppressed reply, or BUM fallback)"
            ...
            <>
              ...
              {state.replicaStage !== "none" && (
                <GlassPanel className="p-4">
                  <p className="text-xs text-pv-text-muted">Switch to 3D View to watch the simultaneous VXLAN replicas travel independently toward each remote leaf.</p>
                </GlassPanel>
              )}
          ...
          {currentStep?.id === "cost-visible" && (
            <BeforeAfterMetrics title="One Discovery Event" beforeLabel="THIS REQUEST" afterLabel="COST" metrics={[{ label: "Original ARP frames", before: 1, after: 1, improved: false }, { label: "VXLAN replicas", before: "—", after: state.lastReplicaCount || 2 }, { label: "Remote VTEPs touched", before: "—", after: state.lastVtepsTouched || 2 }]} />
          )}

          {showBeforeAfter && (
            <BeforeAfterMetrics title="Efficiency Dashboard — Same ARP Question" beforeLabel="WITHOUT SUPPRESSION" afterLabel="WITH SUPPRESSION" metrics={[{ label: "Local ARP frames", before: 1, after: 1, improved: false }, { label: "VXLAN replicas", before: 2, after: 0 }, { label: "Remote VTEPs touched", before: 2, after: 0 }]} />
          )}

          {index >= evpnArpNdSteps.findIndex((s) => s.id === "unknown-binding-case") && index < evpnArpNdSteps.findIndex((s) => s.id === "stale-binding-warning") && (
            <GlassPanel className="p-4">
              <p className="text-xs text-pv-text-muted">Target: <span className="pv-mono text-pv-text">{UNKNOWN_TARGET_IP}</span> — no binding exists on LEAF1 for this address.</p>
            </GlassPanel>
          )}

          {index >= evpnArpNdSteps.findIndex((s) => s.id === "type2-type3-relationship") && index < evpnArpNdSteps.findIndex((s) => s.id === "break-intro") && (
            <GlassPanel className="p-4">
              <div className="grid gap-2 sm:grid-cols-2">
                <div className="rounded-lg border border-pv-cyan/30 bg-pv-cyan/5 p-3"><p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-pv-cyan-soft">Type 2 — &quot;I know the endpoint&quot;</p><p className="text-xs text-pv-text-muted">→ suppress discovery</p></div>
                <div className="rounded-lg border border-pv-violet/30 bg-pv-violet/5 p-3"><p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-pv-violet">Type 3 — &quot;I know the VNI participants&quot;</p><p className="text-xs text-pv-text-muted">→ handle BUM when suppression can&apos;t answer</p></div>
              </div>
            </GlassPanel>
          )}
          ...
          {showPlanes && !isComplete && (
            <PlaneSplitPanel
              show={planeView}
              controlTitle="Control → Discovery — EVPN Enables Local Suppression"
              controlRows={[
                { label: "BGP EVPN mesh", value: state.bgpSessionUp ? "Established" : "Not yet formed" },
                { label: "HOST-B binding on LEAF1", value: `${HOST_B_IP} → ${state.hostBLocation}` },
                { label: "Suppression", value: state.suppressionEnabled ? (state.faultActive ? "Enabled, currently degraded" : "Active") : "Not yet introduced" },
              ]}
              dataTitle="Data Plane — Eventual HOST-A → HOST-B Traffic"
              dataRows={[{ label: "Path", value: `HOST-A → LEAF1 → VXLAN → ${state.hostBLocation} → HOST-B` }, { label: "Unaffected by suppression?", value: "Yes — only discovery changed" }]}
            />
          )}
          ...
          {isComplete && (
            <GlassPanel strong glow="success" className="flex flex-col items-center gap-4 p-10 text-center">
              <Badge tone="success">Broadcast Tamer</Badge>
              <h2 className="text-2xl font-semibold text-pv-text">The Fabric Stopped Asking Questions It Already Answered</h2>
              <p className="max-w-md text-sm text-pv-text-muted">
                You watched LEAF1 suppress an ARP request using its own EVPN MAC/IP binding, confirmed it correctly
                falls back to BUM for an unknown target, then repaired a missing-IP-information fault that degraded
                suppression without ever breaking final connectivity. +350 XP awarded.
              </p>
            ...
            currentStep?.question ? (
              ...
              <div className="space-y-3">
                ...
                <HopInspectorPanel
                  ...
                  onFocusNextHop={(id) => {
                    setSelectedNodeId(id as EvpnArpNdDeviceId);
                    setInspectorSurface("hop");
                    if (cameraMode === "device") setEnteredDeviceId(id as EvpnArpNdDeviceId);
                  }}
                />
...
function RepairChallenge({ options, attempt, onTry }: { options: { id: string; label: string }[]; attempt?: { choice: string; correct: boolean }; onTry: (choice: string) => void }) {
  return (
    <GlassPanel strong className="p-5">
      <p className="mb-3 text-sm font-medium text-pv-text">Reduce unnecessary fabric flooding — choose the correct repair:</p>
      <div className="grid gap-2 sm:grid-cols-2">
        {options.map((opt) => {
          const isSelected = attempt?.choice === opt.id;
          const isCorrect = opt.id === "repair-binding";
          return (
      ...
      {attempt && (
        <div className={clsx("mt-4 rounded-xl border p-4 text-sm", attempt.correct ? "border-pv-success/50 bg-pv-success/10 text-pv-success" : "border-pv-warning/40 bg-pv-warning/5 text-pv-text-muted")}>
          {attempt.correct ? <span className="font-semibold">✓ MAC/IP binding IP information restored — suppression active again.</span> : (<><span className="mb-1 block font-semibold text-pv-text">That doesn&apos;t fix it.</span>{WRONG_FEEDBACK[attempt.choice] ?? "Try again."}</>)}
        </div>
      )}
    </GlassPanel>
  );
