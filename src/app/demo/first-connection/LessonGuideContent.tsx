...
function Device({ x, y, label, sub, accent = C.cyan, w = 104 }: { x: number; y: number; label: string; sub?: string; accent?: string; w?: number }) {
  return (
    <g>
...
function Arrow({ x1, y1, x2, y2, color, dashed, label, labelDy = -8, id }: { x1: number; y1: number; x2: number; y2: number; color: string; dashed?: boolean; label?: string; labelDy?: number; id: string }) {
  return (
    <g>
      <defs>
...
function Svg({ h, children, label }: { h: number; children: ReactNode; label: string }) {
  return (
...
function HeaderField({ label, value, note, color, strong }: { label: string; value: string; note: string; color: string; strong?: boolean }) {
  return (
