...
import { HVPLS_BRIEFING_NOTES, HVPLS_BRIEFING_PHASES } from "./briefing";
import { HVPLS_LESSON_SECTIONS, HvplsLessonGuideContent } from "./LessonGuideContent";
import { HVPLS_DEEP_DIVE_SECTIONS, HvplsDeepDiveContent } from "./DeepDiveContent";

const GUIDE_TABS: LessonGuideTab[] = [
  { id: "lesson", label: "This Lesson", hint: "MTU1–3 spokes · PE1–PE3 core mesh", sections: HVPLS_LESSON_SECTIONS, content: <HvplsLessonGuideContent /> },
  { id: "deep", label: "H-VPLS Deep Dive", hint: "Hierarchical VPLS in general", sections: HVPLS_DEEP_DIVE_SECTIONS, content: <HvplsDeepDiveContent /> },
];
...
export default function HVplsDemo() {
  ...
  const floodCopyLabel = (c: { toId: string }) => floodCopyTitle(state.packet?.frame.dstMac ?? frameDstMac(activePacket), c.toId);
  ...
  const briefing = currentStep ? resolveBriefing(currentStep.id, currentStep.label, HVPLS_BRIEFING_PHASES, HVPLS_BRIEFING_NOTES) : undefined;
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-10">
      ...
      <div>
        ...
        <LessonGuideDialog open={guideOpen} onClose={() => setGuideOpen(false)} title="Hierarchical VPLS (H-VPLS)" subtitle="CUST-A-HVPLS · 3 MTU-s · 3 PE-rs" tabs={GUIDE_TABS} />
      </div>
