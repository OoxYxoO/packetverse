...
Level-3 is a **capability contract**, not a requirement that every lesson share one implementation. Historical (timeline) inspection currently has three implementation shapes:

1. **32 `ScenarioEngine` lessons — frozen `engine.getStateAt(index)` inspection** (§18). This is the current standard, mandatory for new or modernized `ScenarioEngine` lessons.
2. **`sr-mpls-foundations` — the original journey-selection model** (§17): clicking an earlier `HopTimeline` entry selects that router and inspects it against the *live* state; it has no `historicalIndex` and does not call `getStateAt`. This is a known legacy implementation difference awaiting normalization (§18, "Known Level-3 normalization work") — not evidence that live-state history is an acceptable pattern for new work.
3. **`evpn-troubleshooting` — non-linear investigation history** (§19): an intentional Level-3 variant, not built on `ScenarioEngine`, that derives history from its own frozen test results.
...
| Semi-realistic devices, Question Context Mode, original Focus Mode/Hop Inspector | `network3d/devices/*` + `sr-mpls-foundations` (see §17 — its timeline history is the legacy model, not the current standard) |
...
*Historical context / current legacy exception:* the original Focus Mode implementation modeled that presentation-only cursor as plain router selection — clicking an earlier entry points `selectedNodeId`/`enteredDeviceId` at that hop's router, and `traceFor` reads that router's record off the *live* `state.journey`, with no frozen engine state. **`sr-mpls-foundations` still uses this model today.** It is weaker than it looks: anything `traceFor` derives from live state (whether the router is the current hop, which of its hops is latest) reflects *now*, not the clicked moment, and the timeline's selected chip stays on the latest entry. Later Level-3 migrations replaced it with `historicalIndex` + `engine.getStateAt(index)` + the same `traceFor`/`interfacesFor` fed the frozen state — **§18 is the current standard**; do not copy the selection-only model into a new or modernized lesson.
...
This section documents the curriculum-wide standard (mandatory, see `docs/LESSON-BLUEPRINT.md` for the content-level checklist) that every lesson delivers the richest of three capability tiers its real domain data supports — never a lower tier from neglect, never a higher one by inventing data. Every current demo lesson is classified Level 3 — 32 through the standard `ScenarioEngine` pattern below, `evpn-troubleshooting` through the investigation variant in §19, and `sr-mpls-foundations` through the original §17 model pending normalization (see "Known Level-3 normalization work" at the end of this section). The tier definitions remain the standard for any future lesson and still matter conceptually: Level 3 builds on Levels 1 and 2 rather than replacing them.
...
**Known Level-3 normalization work (implementation debt, not a tier gap)**: these don't change the 34 / 34 classification, but they are the remaining follow-ups to bring every `ScenarioEngine` lesson onto the one current standard:

- migrate `sr-mpls-foundations` from live journey-selection inspection (§17) to frozen `historicalIndex` + `getStateAt` inspection;
