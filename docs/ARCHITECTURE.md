...
**Known limitation — `DeviceExplorerPanel` is unreachable inside Focus Mode**: in every current Level 3 lesson, the Focus Mode inspector chain checks `focusTrace` (via `traceFor`, which always falls back to a non-`undefined` `idleTrace(...)`) *before* the `inDeviceMode ? <DeviceExplorerPanel> : ...` branch. Since `focusInspectDeviceId` is set whenever `inDeviceMode` is true, `focusTrace` is therefore always truthy too, so `DeviceExplorerPanel` (and its LIB/LFIB/VRF/CLI/hardware tabs) never actually renders inside Focus Mode — only `HopInspectorPanel` does. Those tabs remain reachable in the main-column (non-Focus-Mode) device view. This is a pre-existing condition of the Focus Mode inspector-chain ordering established in Batch A, not something any individual lesson migration introduced; fixing it would mean either reordering the chain (showing `DeviceExplorerPanel` when a device is entered and nothing more specific is selected) or having `focusTrace` become `undefined` for a genuinely idle device — a shared-component change affecting every Level 3 lesson at once, out of scope for a single lesson's migration.
