...
| SR-MPLS Foundations | `app/demo/sr-mpls-foundations/` | The original Focus Mode / Hop Inspector / Question Context Mode architecture. Its timeline history uses frozen `getStateAt()` inspection, with each chip mapped to the ScenarioEngine step that recorded it across the scenario's `state.journey` resets. |
...
All 34 current demo lessons are classified Level 3. For historical inspection, all 33 `ScenarioEngine` lessons use the standard `getStateAt()` model and `evpn-troubleshooting` uses its intentional investigation-history variant — see ARCHITECTURE.md's curriculum status and §18.
